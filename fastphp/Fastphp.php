...
class Fastphp
{
	...
	public function route()
	{
		...
		// 如果控制器和操作名存在，则实例化控制器，
		// 因为控制器对象里面还会用到控制器和操作名，
		// 所以实例化的时候吧他们两个的名称也传进去。
		// 结合controller基类一起看
		$dispatch = new $controller($controllerName, $actionName);

		// $dispatch保存控制器实例化后的对象，
		// 我们就可以调用他的方法，
		// 也可以向方法中传入参数，
	...
	// 检测自定义全局变量并移除。因为
	// register_globals 已经弃用，如果已经弃用的
	// register_globals 指令被设置为 on，那么局部
	// 变量也将在脚本的全局作用域中可用。
	// 例如，$_POST['foo'] 也将以 $foo 的形式
	// 存在，这样写是不好的实现，会影响代码中的其
	// 他变量。相关信息，
	...
	public function loadClass($className)
	{
		...
		if (isset($classMap[$className])) {
			...
			$file = APP_PATH . str_reqlace('\\', '/', $className) . '.php';
			if (!is_file($file)) {
				return;
			}
		} else {
		...
		// 这里可以加入判断， 如果名为$className的类、接口或者性状不存在，则调试模式下抛出错误
	}
